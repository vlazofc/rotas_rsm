...
await page.route("http://127.0.0.1:8088/api/**", async r => {
  const u = new URL(r.request().url());
  const apiPath = u.pathname.replace(/^\/api/, "");
  let body = [];
  if (apiPath === "/auth/me") body={id:10,email:"motorista@exemplo.com",name:"Motorista Demonstração",role:"motorista",branch_id:1,tenant_id:1,permissions:["module.routes","module.occurrences","module.tracking"],navigation_layout:"sidebar",must_change_password:false};
  else if (apiPath === "/tenants/me") body={feature_rastreamento:true,feature_route_optimization:true,feature_km_calculation:true};
  else if (apiPath === "/tracking/app-permission-consent") body={accepted:true};
  else if (apiPath === "/routes") body=[routeDemo];
  else if (apiPath === "/routes/101") body=routeDemo;
  else if (apiPath === "/drivers") body=[{id:10,name:"Motorista Demonstração",active:true}];
  else if (apiPath === "/vehicles") body=[{id:20,plate:"ABC1D23",active:true}];
  else if (apiPath.includes("failure-reasons")) body=[{id:1,code:"cliente_fechado",label:"Cliente fechado",active:true}];
  else if (apiPath.includes("occurrence-categories")) body=[{id:1,code:"atraso",name:"Atraso",active:true},{id:2,code:"avaria",name:"Avaria",active:true},{id:3,code:"outros",name:"Outros",active:true}];
  else if (apiPath === "/erp/occurrences") body=[{id:1,route_id:101,codigo_ut:"ROTA DEMONSTRAÇÃO",plate:"ABC1D23",driver:"Motorista Demonstração",category:"atraso",severity:"media",description:"Trânsito intenso no acesso ao cliente.",status:"aberta",created_at:"2026-09-15T13:10:00Z"}];
  else if (apiPath === "/notifications/live" || apiPath === "/notifications") body=[];
  else if (apiPath === "/branding/public") body={};
...
await page.goto("http://127.0.0.1:8088/occurrences", { waitUntil: "networkidle" });
