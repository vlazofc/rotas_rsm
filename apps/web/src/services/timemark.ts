...
export async function applyTimemark(file:File,meta:EvidenceContext={},onStage?:(stage:EvidenceStage)=>void):Promise<File>{
 ...
 ctx.fillStyle="rgba(23,23,23,.68)";ctx.fillRect(canvas.width-side,0,side,canvas.height);ctx.fillStyle=YELLOW;ctx.fillRect(canvas.width-side,0,Math.max(5,6*scale),canvas.height);
 ...
 const gradient=ctx.createLinearGradient(0,canvas.height-panel,0,canvas.height);gradient.addColorStop(0,"rgba(23,23,23,.04)");gradient.addColorStop(.18,"rgba(23,23,23,.28)");gradient.addColorStop(1,"rgba(23,23,23,.58)");ctx.fillStyle=gradient;ctx.fillRect(0,canvas.height-panel,canvas.width-side,panel);
 ...
 let y=ly+lh+32*scale;ctx.fillStyle="#fff";ctx.textBaseline="alphabetic";ctx.shadowColor="rgba(0,0,0,.9)";ctx.shadowBlur=Math.max(2,3*scale);for(const [i,line] of lines.entries()){const size=(i===0?24:19)*scale;ctx.font=`${i===0||i===lines.length-1?700:500} ${Math.max(15,size)}px sans-serif`;for(const part of wrap(ctx,line,maxW,2)){ctx.fillText(part,lx,y);y+=Math.max(23,size*1.3)}}ctx.shadowColor="transparent";ctx.shadowBlur=0;
